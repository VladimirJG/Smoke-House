create table Cigarettes
(
    id              int primary key generated by default as identity,
    name            varchar(100)              not null,
    price           int check ( price > 0 ) check ( price < 1000 ),
    description     varchar(200)              not null,
    date_of_issue   date default current_date not null,
    shelf_life_year int check ( shelf_life_year > 1 ),
    additives       varchar(200),
    strength        varchar                   not null
);
create table Liquid
(
    id               int primary key generated by default as identity,
    name             varchar(100)              not null,
    price            int check ( price > 0 ) check ( price < 5000 ),
    description      varchar(200)              not null,
    date_of_issue    date default current_date not null,
    shelf_life_year  int check ( shelf_life_year > 2015 ),
    volume           int check ( volume > 200 ) check ( volume < 1000 ),
    strength         varchar                   not null,
    nicotine_content int,
    taste            varchar(100)
);
create table Disposable
(
    id               int primary key generated by default as identity,
    name             varchar(100)              not null,
    price            int check ( price > 0 ) check ( price < 1000 ),
    description      varchar(200)              not null,
    date_of_issue    date default current_date not null,
    shelf_life_year  int check ( shelf_life_year > 1 ),
    color            varchar(50)               not null,
    strength         varchar                   not null,
    nicotine_content int,
    taste            varchar(100),
    number_of_puffs  int check ( number_of_puffs > 200 ) check ( number_of_puffs < 1000 )
);
create table Vapes
(
    id              int primary key generated by default as identity,
    name            varchar(100) not null,
    price           int check ( price > 0 ) check ( price < 1000 ),
    description     varchar(200) not null,
    date_of_issue   date         not null,
    shelf_life_year int check ( shelf_life_year > 2015 ),
    color           varchar(50)  not null,
    power           int check ( 100 ),
    tank_size       int check ( tank_size > 200 ) check ( tank_size < 500 )
);


insert into Cigarettes(name, price, description, date_of_issue, shelf_life_year, additives, strength)
VALUES ('Parlament', 239, 'Ультратонкие сигареты с насыщенным вкусом и приятным ароматом',
        '2016-12-17', 2, 'Ментол', 'LIGHT');
insert into Cigarettes(name, price, description, date_of_issue, shelf_life_year, additives, strength)
VALUES ('Winston', 185, 'Крепкие сигареты из лучших сортов табака Гватемалы',
        '2023-01-31', 3, 'Без добавок', 'STRONG');
insert into Cigarettes(name, price, description, date_of_issue, shelf_life_year, additives, strength)
VALUES ('Русский стиль', 201, 'Средней крепости сигареты для мидлов',
        '2020-02-12', 2, 'Арабский табак', 'MIDDLE');
insert into Cigarettes(name, price, description, date_of_issue, shelf_life_year, additives, strength)
VALUES ('Kent Brilliance', 179, 'Аромат кофе',
        '2022-09-23', 2, 'Кофе', 'LIGHT');

create table Users
(
    id            int primary key GENERATED by default as identity,
    name          varchar(100) unique       not null,
    date_of_birth date default current_date not null
);

insert into Users(name, date_of_birth)
VALUES ('Хиват Джагуфарович', '1954-04-06');
insert into Users(name, date_of_birth)
VALUES ('Рамзес Александрович', '2000-09-13');
insert into Users(name, date_of_birth)
VALUES ('Андрей Константинович', '1985-03-14');

set datestyle to European;
show datestyle;

select *
from Cigarettes;
select *
from Users;
drop table Cigarettes;
drop table Users;
truncate table Cigarettes;

create table Users_Cigarettes
(
    user_id      int references Users (id),
    cigarette_id int references Cigarettes (id),
    primary key (user_id, cigarette_id)
);

insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (1, 1);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (1, 2);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (1, 3);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (1, 4);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (2, 3);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (2, 4);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (3, 3);
insert into Users_Cigarettes(user_id, cigarette_id)
VALUES (3, 1);

select Users.name
from Users
         join Users_Cigarettes UC on Users.id = UC.user_id
         join Cigarettes C on C.id = UC.cigarette_id
where cigarette_id = 1;

truncate table Users_Cigarettes;

alter table Cigarettes
    add quantity int;

insert into Disposable(name, price, description, date_of_issue, shelf_life_year, color, strength, nicotine_content,
                       taste, number_of_puffs)
values ('Panama', 150, 'Небольшая никотиновая палочка', '1920-12-03', 2, 'Yellow', 'MIDDLE', 3, 'Моченая вишня', 300);
insert into Disposable(name, price, description, date_of_issue, shelf_life_year, color, strength, nicotine_content,
                       taste, number_of_puffs)
values ('Moon', 120, 'Маленькая изящная трубка цвета луны', '1923-04-23', 2, 'White moon', 'LIGHT', 3, 'Жареный карась в помидорах', 250);
insert into Disposable(name, price, description, date_of_issue, shelf_life_year, color, strength, nicotine_content,
                       taste, number_of_puffs)
values ('Lucky', 300, 'Большая сигарилла с мощным давлением', '1922-12-13', 2, 'Silver Hand', 'STRONG', 6, 'Вкус прыжка с парашютом с добавлением малинового варенья', 888);


create table Users_Disposable(
                                 user_id int references Users(id),
                                 disposable_id int references Disposable(id),
                                 primary key (user_id,disposable_id)
);
alter table Disposable
    add quantity int;

truncate table Disposable;
select *from Disposable;
drop table Users_Disposable